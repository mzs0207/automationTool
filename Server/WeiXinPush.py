...
def get_token_form_db():
    try:
        #print 'run get_token_form_db'
        #print 'Host:',config['WX_MySQL_Host']
        #print 'Port:',config['WX_MySQL_Port']
        #print 'User:',config['WX_MySQL_User']
        #print 'Passwd:',config['WX_MySQL_Passwd']
        conn = MySQLdb.connect(host = config['WX_MySQL_Host'], 
                               port = config['WX_MySQL_Port'],
                               user= config['WX_MySQL_User'],
                               passwd = config['WX_MySQL_Passwd'],
                               db='stock_app')
        cursor = conn.cursor()
        #print 'sql:',config['WX_SQL']
        cursor.execute(config['WX_SQL'])
        data = cursor.fetchone()
        cursor.close()
        conn.close()
        #print 'db token:',data[0]
        return data[0]
    except Exception,e:
        print e
        traceback.print_exc()
...
def send_weixin_msg( title,content,remark,users=USERS):
    ...
    if config['WX_Token_Source'] == 'db':
        #print 'source from db'
        token = get_token_form_db()
    else:
        #print 'source from get'
        token= get_token()
    #print 'token:',token
